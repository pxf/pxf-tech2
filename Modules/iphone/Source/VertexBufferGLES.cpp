...
VertexBufferGLES::VertexBufferGLES(GraphicsDevice* _pDevice, VertexBufferLocation _VertexBufferLocation, VertexBufferUsageFlag _VertexBufferUsageFlag)
...
void VertexBufferGLES::_PreDraw()
{
	if(m_VertexAttributes.NumComponents == 0)
		return;

	PXFGLCHECK("VertexBufferGLES::_PreDraw/Start");
	...
	PXFGLCHECK("VertexBufferGLES::_PreDraw/End");
}

void VertexBufferGLES::_PostDraw()
{
	PXFGLCHECK("VertexBufferGLES::_PostDraw/Start");
	if (m_VertexBufferLocation == VB_LOCATION_GPU)
	{
	...
	PXFGLCHECK("VertexBufferGLES::_PostDraw/End");
}



void VertexBufferGLES::CreateNewBuffer(uint32 _NumVertices, uint32 _VertexSize)
{
	PXFGLCHECK("VertexBufferGLES::CreateNewBuffer/Start");
	...
	PXFGLCHECK("VertexBufferGLES::CreateNewBuffer/End");
}

void VertexBufferGLES::CreateFromBuffer(void* _Buffer,uint32 _NumVertices, uint32 _VertexSize)
{
	PXFGLCHECK("VertexBufferGLES::CreateFromBuffer/Start");
	if (m_InterleavedData != 0 && m_BufferObjectId != 0)
		return;
	...
	m_VertexSize = _VertexSize;
	m_ByteCount = _NumVertices * _VertexSize;
	PXFGLCHECK("VertexBufferGLES::CreateFromBuffer/End");
}

void VertexBufferGLES::UpdateData(void* _Buffer, uint32 _Count, uint32 _Offset)
{
	PXF_ASSERT(m_IsMapped == false, "Vertex buffer is busy.");
	PXFGLCHECK("VertexBufferGLES::UpdateData/Start");
	...
	PXFGLCHECK("VertexBufferGLES::UpdateData/End");
}

void* VertexBufferGLES::MapData(VertexBufferAccessFlag _AccessFlag)
{
	PXFGLCHECK("VertexBufferGLES::MapData/Start");
	if (m_VertexBufferLocation == VB_LOCATION_GPU && m_BufferObjectId != 0)
	{
		...
		GL::BindBuffer(GL::ARRAY_BUFFER, 0);
		m_IsMapped = true;
		PXFGLCHECK("VertexBufferGLES::MapData/End");
		return data;
	}
	else if (m_VertexBufferLocation == VB_LOCATION_SYS && m_InterleavedData != 0)
	{
...
void VertexBufferGLES::UnmapData()
{
	PXFGLCHECK("VertexBufferGLES::UnmapData/Start");
	if (m_VertexBufferLocation == VB_LOCATION_GPU && m_BufferObjectId != 0)
	{
		GL::BindBuffer(GL::ARRAY_BUFFER, (GLuint) m_BufferObjectId);
	...
	PXFGLCHECK("VertexBufferGLES::UnmapData/End");
}

void VertexBufferGLES::SetCustomData(uint32 _Index, DeviceDataType _Type, bool _Normalized, uint8 _StrideOffset, uint8 _NumComponents)
{
