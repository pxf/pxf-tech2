...
bool find_any_intersection_closer_than(batch_blob_t *datablob, ray_t *ray, float max_distance, intersection_response_t *resp)
{
	...
	{
		// test intersection
		if (datablob->pData[i]->Intersect(ray, &closest_resp))
		{
...
bool find_intersection(batch_blob_t *datablob, ray_t *ray, Prim *prim, intersection_response_t *resp)
{
	// Loop geometry
	float closest_depth = 100000000000000.0f;
	bool found = false;
	...
	Prim* p = RayTreeIntersect((*datablob->tree),(*ray),10000.0f,(*resp));
	...
	for(size_t i=0; i<datablob->prim_count; i++)
	{
		if (datablob->pData[i]->Intersect(ray,&closest_resp))
		{
			...
			for(int y = 0; y < light->num_rays; ++y)
			{
				for(int x = 0; x < light->num_rays; ++x)
				{
					...
					if (find_any_intersection_closer_than(datablob, &light_ray, light_distance, &tresp))
					{
						...
						// TODO: add better contributing calculations
						float ndotl = Dot(*n, light_ray.d);
						*res += Vec3f(1.0f, 1.0f, 1.0f) * (datablob->lights[l]->material.diffuse * ndotl * ((1.0f / ((float)light->num_rays * (float)light->num_rays)))) / (float)datablob->light_count;
					}
				}
			}
		}
...
{	
	...
	//cray.o = ((SimpleCamera*) datablob->cam)->GetPos();
	//cray.o.z = 1.0f;
	...
	for(int pixel_x = 0; pixel_x < datablob->samples_per_pixel; ++pixel_x)
	{
		for(int pixel_y = 0; pixel_y < datablob->samples_per_pixel; ++pixel_y)
		{
			...
			//bool find_intersection(batch_blob_t *datablob, ray_t *ray, Prim *prim, intersection_response_t *resp)
			if (find_intersection(datablob, &ray, cp, &closest_resp))
			{
				Pxf::Math::Vec3f light_contrib(0.0f, 0.0f, 0.0f);
				Pxf::Math::Vec3f eye_dir = ray.o - closest_resp.p;
				Normalize(eye_dir);
				
				fpixel += Vec3f(0.1f, 0.1f, 0.1f); //closest_prim->material.ambient;
				...
				*/
				...
				float refl = 0.0f; //closest_prim->material.reflectiveness;
