...
int Fabric::app_getwindimensions(lua_State *L)
{
  // w,h = app.getwindimensions() -- w,h = width and height of window
  if (lua_gettop(L) == 0)
  {
    Pxf::Graphics::Window* wnd = App::GetInstance()->m_win;
...
int Fabric::app_quit(lua_State *L)
{
...
int Fabric::app_savedialog(lua_State *L)
{
...
int Fabric::app_opendialog(lua_State *L)
{
...
int Fabric::app_getcwd(lua_State* L)
{
	if (lua_gettop(L) == 0)
	{
		char path[261];
...
int Fabric::app__setrenderoption(lua_State *L)
{
  // app.setrenderoption(render_mode) -- changes render option
  if (lua_gettop(L) == 1)
  {
    App::GetInstance()->m_RedrawMode = lua_tointeger(L, 1);
    return 0;
...
int Fabric::app__getrenderoption(lua_State *L)
{
  // render_mode = app.getrenderoption() -- get render option
  if (lua_gettop(L) == 0)
  {
    lua_pushnumber(L, App::GetInstance()->m_RedrawMode);
    return 1;
...
int Fabric::util_hashfile(lua_State *L)
{
	unsigned long ret = 0;
	if (lua_gettop(L) == 1)
	{
...
int Fabric::util_pathext(lua_State *L)
{
	if (lua_gettop(L) == 1)
	{
		const char* filename = lua_tostring(L, 1);
...
int Fabric::util_pathfilename(lua_State *L)
{
	if (lua_gettop(L) == 1)
	{
		const char* filename = lua_tostring(L, 1);
...
int Fabric::luaopen_appcore (lua_State *L) {
  ...
  lua_pushinteger(L, FABRIC_REDRAWMODE_FULL);
  lua_setfield(L, -2, "REDRAWMODE_FULL");
  lua_pushinteger(L, FABRIC_REDRAWMODE_NORMAL);
