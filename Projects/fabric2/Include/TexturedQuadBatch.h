...
namespace Fabric
{
    ...
    public:
        TexturedQuadBatch(unsigned int _size, const char* _texture_filepath, float* _currentdepth, Pxf::Math::Vec4f* _currentcolor, Pxf::Math::Mat4* _transformmatrix = NULL, bool _linear = false);
				...
				TexturedQuadBatch(unsigned int _size, int _width, int _height, int _channels, Pxf::Graphics::Texture* _texture, float* _currentdepth, Pxf::Math::Vec4f* _currentcolor, Pxf::Math::Mat4* _transformmatrix = NULL, bool _linear = false);
